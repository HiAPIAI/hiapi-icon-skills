...
function targets(args) {
  ...
  if (args.includes("--codex") || existsSync(codex)) candidates.push({ label: "Codex", dir: join(codex, "skills") });
  const claude = join(homedir(), ".claude");
  if (args.includes("--claude") || existsSync(claude)) candidates.push({ label: "Claude Code", dir: join(claude, "skills") });
